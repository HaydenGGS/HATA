...
class predict_net(nn.Module):
    ...
    def get_log_p(self, own_variable, other_variable):
...
class predict_net_withtrait(nn.Module):

    def __init__(self, num_inputs, hidden_dim, num_outputs, layer_norm=True, lr=1e-3):
        super(predict_net_withtrait, self).__init__()
    ...
    def forward(self, input, traits):
        if self.layer_norm:
            h = F.relu(self.ln1(self.linear1(input)))
        else:
            h = F.relu(self.linear1(input))

        h = torch.cat([h, traits], dim=-1)
        h = F.relu(self.linear2(h))
        x = self.last_fc(h)
        return x

    def get_log_p(self, own_variable, other_variable, add_id):
